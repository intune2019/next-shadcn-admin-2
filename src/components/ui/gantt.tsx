...
import { addDays, addMonths, differenceInDays, differenceInHours, differenceInMonths, endOfDay, endOfMonth, format, formatDate, formatDistance, getDate, getDaysInMonth, isSameDay, startOfDay, startOfMonth } from "date-fns";
...
const getsDaysIn = (range: Range) => (range === "daily" ? () => 1 : getDaysInMonth);

const getDifferenceIn = (range: Range) => (range === "daily" ? differenceInDays : differenceInMonths);

const getInnerDifferenceIn = (range: Range) => (range === "daily" ? differenceInHours : differenceInDays);
...
const getAddRange = (range: Range) => (range === "daily" ? addDays : addMonths);
  ...
  for (const yearObj of data) {
    yearObj.quarters = new Array(4).fill(null).map((_, quarterIndex) => ({
      months: new Array(3).fill(null).map((_, monthIndex) => ({
        days: getDaysInMonth(new Date(yearObj.year, quarterIndex * 3 + monthIndex, 1)),
      })),
    }));
  }
...
const getOffset = (date: Date, timelineStartDate: Date, context: GanttContextProps) => {
  ...
  const pixelsPerDay = parsedColumnWidth / getDaysInMonth(date);
...
const getWidth = (startAt: Date, endAt: Date | null, context: GanttContextProps) => {
  ...
  const pixelsPerDayInStartMonth = parsedColumnWidth / getDaysInMonth(startAt);
  if (isSameDay(startAt, endAt)) return pixelsPerDayInStartMonth;

  const innerDifferenceIn = getInnerDifferenceIn(context.range);
  const startOf = getStartOf(context.range);
  if (isSameDay(startOf(startAt), startOf(endAt))) return innerDifferenceIn(endAt, startAt) * pixelsPerDayInStartMonth;

  const startRangeOffset = getDaysInMonth(startAt) - getDate(startAt);
  const endRangeOffset = getDate(endAt);
  const fullRangeOffset = differenceIn(startOf(endAt), startOf(startAt));
  const pixelsPerDayInEndMonth = parsedColumnWidth / getDaysInMonth(endAt);
...
const calculateInnerOffset = (date: Date, range: Range, columnWidth: number) => {
  const startOfRange = getStartOf(range)(date);
  const endOfRange = getEndOf(range)(date);
  const totalRangeDays = getInnerDifferenceIn(range)(endOfRange, startOfRange);
  return (date.getDate() / totalRangeDays) * columnWidth;
};
  ...
  return gantt.timelineData.map((year) =>
    year.quarters.flatMap((quarter) => quarter.months).map((month, index) => (
      <div className="relative flex flex-col" key={`${year.year}-${index}`}>
        ...
        />
        <GanttColumns columns={month.days} isColumnSecondary={(item) => [0, 6].includes(addDays(new Date(year.year, index, 1), item).getDay())} />
      </div>
    )),
  );
};
...
const MonthlyHeader: FC = () => {
  ...
  return gantt.timelineData.map((year) => (
    <div className="relative flex flex-col" key={year.year}>
      <GanttContentHeader title={`${year.year}`} columns={year.quarters.flatMap((quarter) => quarter.months).length} renderHeaderItem={(item) => <p>{format(new Date(year.year, item, 1), "MMM")}</p>} />
  ...
  return gantt.timelineData.map((year) =>
    year.quarters.map((quarter, quarterIndex) => (
      <div className="relative flex flex-col" key={`${year.year}-${quarterIndex}`}>
        <GanttContentHeader title={`Q${quarterIndex + 1} ${year.year}`} columns={quarter.months.length} renderHeaderItem={(item) => <p>{format(new Date(year.year, quarterIndex * 3 + item, 1), "MMM")}</p>} />
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({ feature, onSelectItem, className }) => {
  ...
  return (
    <div role="button" onClick={handleClick} onKeyDown={handleKeyDown} tabIndex={0} key={feature.id} className={cn("relative flex items-center gap-2.5 p-2.5 text-xs", className)} style={{ height: "var(--gantt-row-height)" }}>
...
export const GanttSidebarHeader: FC = () => (
  <div className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-border/50 border-b bg-backdrop/90 p-2.5 font-medium text-muted-foreground text-xs" style={{ height: "var(--gantt-header-height)" }}>
...
export const GanttAddFeatureHelper: FC<GanttAddFeatureHelperProps> = ({ top, className }) => {
  ...
  const handleClick = () => {
    ...
    gantt.onAddItem?.(getDateByMousePosition(gantt, x));
  };
...
export const GanttColumn: FC<GanttColumnProps> = ({ index, isColumnSecondary }) => {
  ...
  return (
    <div className={cn("group relative h-full overflow-hidden", isColumnSecondary?.(index) ? "bg-secondary" : "")} ref={mouseRef} onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)}>
...
export const GanttColumns: FC<GanttColumnsProps> = ({ columns, isColumnSecondary }) => {
  const id = useId();
  return (
    <div className="divide grid h-full w-full divide-x divide-border/50" style={{ gridTemplateColumns: `repeat(${columns}, var(--gantt-column-width))` }}>
      {Array.from({ length: columns }).map((_, index) => <GanttColumn key={`${id}-${index}`} index={index} isColumnSecondary={isColumnSecondary} />)}
    </div>
  );
};
  ...
  return (
    <div className={cn("group pointer-events-none absolute top-0 left-0 h-full w-full select-none overflow-visible", className)} ref={mouseRef}>
      <div className="-ml-2 pointer-events-auto sticky top-6 z-20 flex w-4 flex-col items-center justify-center gap-1 overflow-visible opacity-0 group-hover:opacity-100" style={{ transform: `translateX(${x}px)` }}>
        <button type="button" className="z-50 inline-flex h-4 w-4 items-center justify-center rounded-full bg-card" onClick={() => onCreateMarker(getDateByMousePosition(gantt, x))}>
          <PlusIcon size={12} className="text-muted-foreground" />
        </button>
        <div className="whitespace-nowrap rounded-full border border-border/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg">{formatDate(getDateByMousePosition(gantt, x), "MMM dd, yyyy")}</div>
      </div>
    </div>
  );
};
...
export const GanttFeatureDragHelper: FC<GanttFeatureDragHelperProps> = ({ direction, featureId, date }) => {
  ...
  return (
    <div className={cn("group -translate-y-1/2 absolute top-1/2 z-[3] h-full w-6 cursor-col-resize rounded-md outline-none", direction === "left" ? "-left-2.5" : "-right-2.5")} ref={setNodeRef} {...attributes} {...listeners}>
      ...
      {date && <div className={cn("-translate-x-1/2 absolute top-10 hidden whitespace-nowrap rounded-lg border border-border/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg group-hover:block", isPressed && "block")}>{format(date, "MMM dd, yyyy")}</div>}
    </div>
  );
};
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({ onMove, children, className, ...feature }) => {
  ...
  const handleItemDragMove = () => {
    ...
    const delta = gantt.range === "daily" ? getDifferenceIn(gantt.range)(currentDate, originalDate) : getInnerDifferenceIn(gantt.range)(currentDate, originalDate);
  ...
  return (
    <div className={cn("relative flex w-max min-w-full py-0.5", className)} style={{ height: "var(--gantt-row-height)" }}>
      <div className="pointer-events-auto absolute top-0.5" style={{ height: "calc(var(--gantt-row-height) - 4px)", width: Math.round(width), left: Math.round(offset) }}>
        {onMove && <DndContext sensors={[mouseSensor]} modifiers={[restrictToHorizontalAxis]} onDragMove={handleLeftDragMove} onDragEnd={() => onMove(feature.id, startAt, endAt)}><GanttFeatureDragHelper direction="left" featureId={feature.id} date={startAt} /></DndContext>}
        <DndContext sensors={[mouseSensor]} modifiers={[restrictToHorizontalAxis]} onDragStart={() => { setPreviousMouseX(mousePosition.x); setPreviousStartAt(startAt); setPreviousEndAt(endAt); }} onDragMove={handleItemDragMove} onDragEnd={() => onMove?.(feature.id, startAt, endAt)}>
          <GanttFeatureItemCard id={feature.id}>{children ?? <p className="flex-1 truncate text-xs">{feature.name}</p>}</GanttFeatureItemCard>
        </DndContext>
        {onMove && <DndContext sensors={[mouseSensor]} modifiers={[restrictToHorizontalAxis]} onDragMove={handleRightDragMove} onDragEnd={() => onMove(feature.id, startAt, endAt)}><GanttFeatureDragHelper direction="right" featureId={feature.id} date={endAt ?? addRange(startAt, 2)} /></DndContext>}
      </div>
    </div>
  );
};
...
export const GanttFeatureListGroup: FC<GanttFeatureListGroupProps> = ({ children, className }) => <div className={className} style={{ paddingTop: "var(--gantt-row-height)" }}>{children}</div>;

export type GanttFeatureListProps = { className?: string; children: ReactNode };
export const GanttFeatureList: FC<GanttFeatureListProps> = ({ className, children }) => <div className={cn("absolute top-0 left-0 h-full w-max space-y-4", className)} style={{ marginTop: "var(--gantt-header-height)" }}>{children}</div>;
  ...
  return (
    <div className="pointer-events-none absolute top-0 left-0 z-20 flex h-full select-none flex-col items-center justify-center overflow-visible" style={{ width: 0, transform: `translateX(calc(var(--gantt-column-width) * ${offset} + ${innerOffset}px))` }}>
      <ContextMenu>
        ...
        <ContextMenuContent>
          {onRemove ? <ContextMenuItem className="flex items-center gap-2 text-destructive" onClick={() => onRemove(id)}><TrashIcon size={16} />Remove marker</ContextMenuItem> : null}
        </ContextMenuContent>
      </ContextMenu>
...
export const GanttProvider: FC<GanttProviderProps> = ({ zoom = 100, range = "monthly", onAddItem, children, className }) => {
  ...
  const handleScroll = useCallback(
    throttle(() => {
      ...
      if (scrollLeft === 0) {
        ...
        if (!firstYear) return;
        setTimelineData((previous) => [{ year: firstYear - 1, quarters: createInitialTimelineData(new Date(firstYear - 1, 0, 1))[0].quarters }, ...previous]);
        scrollRef.current.scrollLeft = scrollRef.current.clientWidth;
      } else if (scrollLeft + clientWidth >= scrollWidth) {
        const lastYear = timelineData.at(-1)?.year;
        if (!lastYear) return;
        setTimelineData((previous) => [...previous, { year: lastYear + 1, quarters: createInitialTimelineData(new Date(lastYear + 1, 0, 1))[0].quarters }]);
        scrollRef.current.scrollLeft = scrollRef.current.scrollWidth - scrollRef.current.clientWidth;
      }
    }, 100),
    [setScrollX, timelineData],
  );
  ...
  return (
    <GanttContext.Provider value={{ zoom, range, headerHeight, columnWidth, sidebarWidth, rowHeight, onAddItem, timelineData, placeholderLength: 2, ref: scrollRef }}>
      <div className={cn("gantt relative grid h-full w-full flex-none select-none overflow-auto rounded-sm bg-secondary", range, className)} style={{ ...cssVariables, gridTemplateColumns: "var(--gantt-sidebar-width) 1fr" }} ref={scrollRef}>
        {children}
      </div>
...
export const GanttTimeline: FC<GanttTimelineProps> = ({ children, className }) => <div className={cn("relative flex h-full w-max flex-none overflow-clip", className)}>{children}</div>;
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
  ...
  return (
    <div className="pointer-events-none absolute top-0 left-0 z-20 flex h-full select-none flex-col items-center justify-center overflow-visible" style={{ width: 0, transform: `translateX(calc(var(--gantt-column-width) * ${offset} + ${innerOffset}px))` }}>
      <div className={cn("group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md bg-card px-2 py-1 text-foreground text-xs", className)}>Today<span className="max-h-[0] overflow-hidden opacity-80 transition-all group-hover:max-h-[2rem]">{format(date, "MMM dd, yyyy")}</span></div>
      <div className={cn("h-full w-px bg-card", className)} />
    </div>
  );
};
